...
@partial.partial
def associate_by_email_if_register_api(auth_entry, details, *args, **kwargs):
    if auth_entry == AUTH_ENTRY_REGISTER_API and details.get('email'):
        try:
            user = User.objects.get(email=kwargs['details']['email'])
